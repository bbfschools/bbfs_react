...
if ( ! class_exists( __NAMESPACE__ . '\Abstract_Settings' ) ) {
	...
	# region Construction and Initialization
		 ...
		 */
		final public static function is_valid_constructor_args( $plugin, $renderer ) {
		...
		protected function __construct( $plugin, $renderer = null ) {
			...
			if ( ! Lib::is_debug() || self::is_valid_constructor_args( $plugin, $renderer ) ) {
				...
				if ( ! Lib::is_debug() || $this->is_valid_settings_configuration( $this->settings, $this->renderer ) ) {
					...
					register_activation_hook( $file, array( $this, 'on_abstract_activate_plugin' ) );
					register_deactivation_hook( $file, array( $this, 'on_abstract_deactivate_plugin' ) );
					register_uninstall_hook( $file, array( get_class( $this ), 'on_abstract_uninstall_plugin' ) );
	...
	# region Activation requirements, activate, deactivate, uninstall
		 ...
		 * @return void
		 */
		final public function on_abstract_activate_plugin( $networkwide ) {
			...
			$this->on_activate_plugin( $networkwide );
		}
							...
							if ( ! is_array( $requires ['constants'] ) ) {
								...
								foreach ( $requires ['constants'] as $constant => $require_name ) {
									if ( ! is_string( $constant ) ) {
										...
										// Translators: %1$s is PHP constant name, %2$s is require name.
		...
		/** Deactivate plugin.
		 *
		 * Private final, not intended to be overridden from derived class.
		 ...
		 * @return void
		 */
		final public function on_abstract_deactivate_plugin( $networkwide ) {
		...
		/** Uninstall plugin.
		 *
		 * Static final, not intended to be overridden from derived class.
		 ...
		 * @return void
		 */
		final public static function on_abstract_uninstall_plugin() {
	...
	# region Initialization, Enqueue, Setup ....
		 ...
		 * @return void
		 */
		final public function on_abstract_init_action() {
		 ...
		 * @return void
		 */
		final public function on_abstract_admin_enqueue_scripts() {
			...
			Lib::enqueue_script( 'common' );
			Lib::enqueue_script( 'postbox' );
			Lib::enqueue_script( 'wp-lists' );

			Lib::enqueue_script( 'accordion' );

			Lib::enqueue_script( 'utils' );

			Lib::enqueue_script( 'jquery-ui-draggable' );
			Lib::enqueue_script( 'jquery-ui-droppable' );
			Lib::enqueue_script( 'jquery-ui-sortable' );

			Lib::enqueue_script( 'abstract-settings-admin', $relative_path . '/assets/admin.js', array( 'jquery' ), $this->plugin->get_abstract_version() );
			...
			Lib::register_script( 'abstract-settings-admin-styled', $relative_path . '/assets/admin-styled.js', array( 'jquery', 'abstract-settings-jquery-chosen', 'abstract-settings-jquery-are-you-sure' ), $this->plugin->get_abstract_version() );
		 ...
		 * @return void
		 */
		final public function on_abstract_register_settings_menu() {
		 ...
		 * @return void
		 */
		final public function on_abstract_register_settings_page() {
			...
			register_setting(
				$this->pageslug, $this->optionid, array(
					'type'              => 'array',
					'group'             => $this->pageslug,
	...
	# region Render Fields.
		 ...
		 */
		final private function get_field_settings_args( $args ) {
		 ...
		 */
		final private function get_form_field_option_value( $fn, $args ) {
	...
	# region Render Sections
		 ...
		 * @return void
		 */
		final private function render_section_elements( $wp_section, $section_is_open, $sections_todo, $tab_index, $push_tabs = 0 ) {
	...
	# region Render Tab-Page & Elements
		...
		private function render_tab_open_elements( $tab_index, $tab_settings, $tab_sections = false, $push_tabs = 0 ) {
			...
			Lib::echo_html( Lib::debug_eol_tabs( 0 + $push_tabs ) . "<div id=\"nav-tab-page-$tab_index\" class=\"nav-tab-page$tab_class_id\" role=\"tabpanel\" aria-labelledby=\"nav-tab-$tab_index\">" );
			...
			if ( $render_method_name && is_callable( array( $this->renderer, $render_method_name ) ) ) {
				...
				try {
					...
					if ( strlen( $tab_contents ) > 0 ) {
						Lib::echo_html( Lib::debug_eol_tabs( 2 + $push_tabs ) . "<div id=\"nav-tab-page-header-$tab_index\" class=\"nav-tab-page-header$tab_class_id\">" );
			...
			Lib::echo_html( Lib::debug_eol_tabs( 2 + $push_tabs ) . "<div id=\"nav-tab-page-content-$tab_index\" class=\"nav-tab-page-content$tab_class_id\">" );
		}
		 ...
		 * @param int   $push_tabs - add X tabs for debug formatting.
		 */
		final private function render_tab_close_elements( $tab_index = 0, $tab_settings, $tab_sections, $push_tabs = 0 ) {
	...
	# region Render Settings Page
		 ...
		 * @return void
		 */
		final public function on_abstract_prepare_settings_page() {
		 ...
		 * @return void
		 */
		final public function render_settings_page() {
			...
			if ( count( $tabs_settings ) === 0 ) {
				...
				$open_tab_state = $this->pageslug . '-tabindex';
				$open_tab       = (int) get_user_setting( $open_tab_state, 0 );

				$tab_index = 0;
				foreach ( $tabs_settings as $ignore ) {
					Lib::echo_html( Lib::debug_eol_tabs( 5 ) . "<input type=\"radio\" class=\"nav-tab-state\" name=\"nav-tab-state\" style=\"display:none\" id=\"nav-tab-state-$tab_index\" data-ays-ignore=\"true\"" . ( $open_tab === $tab_index ? ' checked' : '' ) . '/>' );
					++ $tab_index;
				}
			...
			$open_sections_state = $this->pageslug . '-sections';
			$open_sections_value = get_user_setting( $open_sections_state, '' );
			$open_sections_items = strlen( $open_sections_value );
			for ( $i = 0; $i < $open_sections_items; $i++ ) {
				$open_sections [ $i ] = ( '1' === $open_sections_value [ $i ] ? true : false );
			}
			...
			if ( count( $tabs_settings ) !== 0 ) {
				...
				foreach ( $wp_sections as $wp_section ) {
					...
					if ( $sections_todo > 0 ) {
						$section_is_open = ( is_bool( $open_sections [ $section_index ] ) ? $open_sections [ $section_index ] : true );
				...
				foreach ( $wp_sections as $wp_section ) {
					...
					$section_is_open = ( ( is_bool( $open_sections [ $section_index ] ) ? $open_sections [ $section_index ] : true ) ? ' open' : '' );
	...
	# region Form Submit handlers (Validation & File Uploads)
		 ...
		 */
		final public function on_abstract_validate_form_input( $new_value, $old_value, $option_id ) {
				...
				&& array_key_exists( $field_name, $fields ) ) {
					...
					if ( false !== $field_type ) {
						...
						if ( false === \is_string( $input_value ) && false === \is_array( $input_value ) ) {
							Lib::debug( 'Input Field is not a string or array?' );
						}
								...
								if ( true === $is_multiple ) {
									...
									foreach ( $input_value as &$input_item_value ) {
										if ( \is_array( $input_item_value ) ) {
											...
											Lib::debug( 'Multiplied Checkbox value is not an array?' );
											$input_accept = false;
										}
									}
								} else {
									if ( \is_array( $input_value ) ) {
										...
										Lib::debug( 'Checkbox value is not an array?' );
									}
								}
								break;
							...
							case 'select':
								if ( true === $is_multiple ) {
									...
									if ( \is_array( $input_value ) ) {
										...
										Lib::debug( 'Select multiple value is not an array?' );
									}
								} else {
									...
									if ( \is_string( $input_value ) ) {
										...
										Lib::debug( 'Select single value is not a string?' );
									}
								}
								break;
							...
							case 'textarea':
								if ( true === $is_multiple ) {
									if ( \is_array( $input_value ) ) {
										...
										Lib::debug( 'Multiplied Textarea is not an array?' );
									}
								} else {
									if ( \is_string( $input_value ) ) {
										...
										Lib::debug( 'Textarea is not a string?' );
									}
								}
								break;
							...
							default:
								if ( true === $is_multiple ) {
									if ( \is_array( $input_value ) ) {
										...
										Lib::debug( 'Multiplied Input Field is not an array?' );
									}
								} else {
									if ( \is_string( $input_value ) ) {
										if ( true === $is_integer ) {
										...
										Lib::debug( 'Input Field is not a string?' );
									}
								}
								break;
						}
			...
			$output = $this->validate_form_input( $output );
	...
	# region Extension Helpers & Utilities
		 ...
		 */
		final public function add_settings_update_error( $msg = '', $code = '' ) {
		 ...
		 * @param string $code - error code, optional.
		 */
		final public function add_settings_update_notice( $msg = '', $code = '' ) {
		 ...
		 */
		final private function add_settings_update_message( $msg, $code = '', $type = 'error' ) {
		 ...
		 */
		final public function get_plugin() {
			return $this->plugin;
		}
		 ...
		 */
		final public function get_all_fields_defaults() {
		 ...
		 * @return array of string => mixed (field-name => default-value)
		 */
		final public function get_all_fields_settings() {
		 ...
		 */
		final public function get_field_settings( $fn ) {
	...
	# region Extension Methods (override in derived class, if needed)
		 ...
		 */
		protected function validate_form_input( $options ) {
			return $options;
		}
	...
	# region Extension Hooks (filters/actions to hook into)
		 ...
		 * @return array $args
		 */
		final private function apply_field_args_filters( $args, $fn ) {
		 ...
		 * @return string $html
		 */
		final private function apply_field_html_filters( $html, $args, $fn ) {
