...
(function ($) {
	if (typeof $ === 'function') {
		function fixChosenContainerWidth () {
			...
			// Hook on tab switch
			$('input[type=radio][name=nav-tab-state]').change(function() {
				$containers.each (function (ix, el) {
			...
			$sections.on ('click keydown', '.accordion-section-title', function (e) {
				if ( e.type === 'keydown' && 13 !== e.which ) { return; }
				var $found = $();
				var $chosen;
				$sections.find ('div.chosen-container').each (function (ix, el) {// Get Chosen containers
					if (el.style.width==='0px') { // Chosen width is 0px?
						$chosen = $(el);
				...
				if ($found.length !== 0) {
					setTimeout (function () { // ?Wait for the animations to finish (150)
						$found.each (function (ix, el) {// Get Chosen containers
							$chosen = $(el);
							// Set Chosen width to width of previous sibling (select) element
							$chosen.width ($chosen.prev ().css ('width'));
							$chosen.show ();
		...
		function adminStyle () {
			...
			$("div.admin-color").css ("color", $color); // For IExplorer CSS variable not supported.
		}
		$(function () {
			adminStyle ();
