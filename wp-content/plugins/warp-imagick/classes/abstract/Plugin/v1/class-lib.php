...
if ( ! class_exists( __NAMESPACE__ . '\Lib' ) ) {
	...
	# region Admin notices
		...
		public static function get_namespace() {
			...
			return strtolower( implode( $namespace, '_' ) );
		}
	...
	# region Debug helpers
		 ...
		 * Returns true if WP_DEBUG is defined and equal to true.
		 *
		 * @access public
		 * @return bool true if WP_DEBUG === true.
		 */
			...
			if ( self::is_debug() ) {
				...
				if ( true === $trace ) {
					...
					foreach ( $stack as $trace ) {
						$echo_message = $trace ['function'] . PHP_EOL . $echo_message;
					}
				} elseif ( is_string( $trace ) && trim( $trace ) ) {
					$echo_message = rtrim( trim( $trace ), ':' ) . ': ' . $echo_message;
				}
